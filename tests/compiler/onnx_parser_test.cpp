...
int main() {
    ...
    auto mm = neuralOS::onnx::lookup_op("MatMul");
    CHECK(mm.supported, "MatMul is supported");
    CHECK(mm.kind == neuralOS::L1::HighOpKind::MATMUL, "MatMul → MATMUL");

    auto attn = neuralOS::onnx::lookup_op("Attention");
    CHECK(attn.kind == neuralOS::L1::HighOpKind::ATTENTION, "Attention → ATTENTION");

    auto unknown = neuralOS::onnx::lookup_op("UnknownOp");
    CHECK(!unknown.supported, "UnknownOp not supported");
    CHECK(unknown.kind == neuralOS::L1::HighOpKind::CUSTOM, "UnknownOp → CUSTOM");

    auto gelu = neuralOS::onnx::lookup_op("Gelu");
    CHECK(gelu.kind == neuralOS::L1::HighOpKind::GELU, "Gelu → GELU");

    auto dq = neuralOS::onnx::lookup_op("DequantizeLinear");
    CHECK(dq.kind == neuralOS::L1::HighOpKind::DEQUANT, "DequantizeLinear → DEQUANT");

    /* Test 2: Op map coverage */
    auto& map = neuralOS::onnx::get_op_map();
    CHECK(map.size() >= 40, "at least 40 ops mapped");

    /* Test 3: Build a synthetic OnnxGraph and convert to NFIR */
    neuralOS::onnx::OnnxGraph onnx_graph;
    onnx_graph.name = "test_model";

    neuralOS::onnx::OnnxNode n1;
    n1.op_type = "MatMul";
    n1.name = "matmul_0";
    ...
    bool ok = neuralOS::onnx::onnx_to_nfir(onnx_graph, &nfir);
    CHECK(ok, "onnx_to_nfir succeeded");
    CHECK(nfir.num_ops() == 3, "3 ops converted");

    /* Verify op kinds */
    CHECK(nfir.ops[0].kind == neuralOS::L1::HighOpKind::MATMUL, "op0 is MATMUL");
    CHECK(nfir.ops[1].kind == neuralOS::L1::HighOpKind::ELEMENT_ADD, "op1 is ELEMENT_ADD");
    CHECK(nfir.ops[2].kind == neuralOS::L1::HighOpKind::GELU, "op2 is GELU");
    ...
    /* Test 4: Null graph */
    CHECK(!neuralOS::onnx::onnx_to_nfir(onnx_graph, nullptr), "null output fails");

    /* Test 5: ProtobufReader basics */
    uint8_t varint_data[] = {0xAC, 0x02};  /* 300 in varint */
    neuralOS::onnx::ProtobufReader reader(varint_data, 2);
