...
public:
    ...
    /* Phase 36: Export block references for vMMU radix prefix sharing.
     * Returns token keys of all cached entries for prefix matching. */
    struct BlockRef {
        std::vector<int32_t> token_key;
        uint64_t             size_bytes = 0;
        uint64_t             seq_id     = 0;
    ...
    std::vector<BlockRef> export_block_refs() const {
        std::shared_lock<std::shared_mutex> lk(mu_);
        std::vector<BlockRef> refs;
        refs.reserve(all_entries_.size());
        for (auto* node : all_entries_) {
            if (node->entry) {
                refs.push_back({
                    node->entry->token_key,
                    node->entry->size_bytes,
                    node->entry->seq_id
                });
            }
        }
